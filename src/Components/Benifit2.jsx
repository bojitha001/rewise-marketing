...
const Benifit2 = () => {
  return (
    <div className='h-screen text-white benifit-2 fixed top-0 left-0 right-0 z-40 overflow-hidden flex flex-col justify-between px-6 md:px-10 lg:px-16 py-8 md:py-12 rounded-2xl md:rounded-3xl' style={{ background: 'linear-gradient(135deg, #0028a0 0%, #0040c1 50%, #1a5cff 100%)' }}>
      {/* Top row */}
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white/70 text-xs md:text-sm font-light tracking-widest uppercase">Benefit 03</span>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="max-w-2xl">
          ...
          <p className="text-white/80 text-base md:text-xl font-light leading-relaxed max-w-md">
          ...
          <div className="mt-6 md:mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <button className="bg-white text-[#0040c1] px-6 md:px-8 py-3 md:py-4 rounded-full font-medium text-base md:text-lg hover:bg-blue-50 transition-colors">
              Start Tracking Free
            </button>
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                <img src="https://i.pravatar.cc/40?img=4" alt="" className="w-8 h-8 md:w-9 md:h-9 rounded-full border-2 border-white/30" />
                <img src="https://i.pravatar.cc/40?img=5" alt="" className="w-8 h-8 md:w-9 md:h-9 rounded-full border-2 border-white/30" />
                <img src="https://i.pravatar.cc/40?img=6" alt="" className="w-8 h-8 md:w-9 md:h-9 rounded-full border-2 border-white/30" />
              </div>
              <span className="text-white/70 text-xs md:text-sm font-light">+10k learners tracking</span>
            </div>
          </div>
        </div>

        {/* Progress widget */}
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 md:p-8 min-w-0 md:min-w-[260px] flex flex-col gap-5">
